...
def create_colors_info(team1_name, team1_p_color, team1_gk_color, team2_name, team2_p_color, team2_gk_color):
    ...
    color_list_lab = [skimage.color.rgb2lab(
        [i/255 for i in c]) for c in colors_list]
...
def detect(cap, stframe, output_file_name, save_output, model_players, model_keypoints,
    ...
    if save_output:
        ...
        output = cv2.VideoWriter(
            f'./outputs/{output_file_name}.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30.0, (width, height))
    ...
    csv_file = open(f'./outputs/{output_file_name}.csv', 'w', newline='')
    csv_writer = csv.writer(csv_file)
    # Write header to CSV
    csv_writer.writerow(['Frame', 'Time (s)', 'Object',
                        'Label', 'Confidence', 'Position (x, y)', 'Team'])
        ...
        if success:
            ...
            if len(detected_labels) > 3:
                ...
                if frame_nbr > 1:
                    ...
                    common_labels = set(
                        detected_labels_prev) & set(detected_labels)
            ...
            if 'homog' in locals():
                ...
                detected_ball_src_pos = bboxes_p_c_2[0,
                                                     :2] if bboxes_p_c_2.shape[0] > 0 else None
                ...
                pred_dst_pts = []
                # Loop over players frame coordiantes
            ...
            # Initialize players color palette list
            obj_palette_list = []
            ...
            for i, j in enumerate(list(results_players[0].boxes.cls.cpu().numpy())):
                if int(j) == 0:
                    ...
                    obj_img = frame_rgb[int(bbox[1]):int(bbox[3]), int(
                        bbox[0]):int(bbox[2])]       # Crop bbox out of the frame
                    ...
                    color_count = [(n, palette[m])
                                   for n, m in reduced.getcolors()]
            ...
            for palette in obj_palette_list:
                ...
                palette_lab = [skimage.color.rgb2lab(
                    [i/255 for i in color]) for color in palette]
            ...
            # Initializing counter of detected players
            j = 0
            # Set color box size in pixels (for display)
            palette_box_size = 10
            ...
            for i in range(bboxes_p.shape[0]):
                # Get confidence of current detected object
                conf = confs_p[i]
                ...
                if labels_p[i] == 0:
                    ...
                    csv_writer.writerow([frame_nbr, current_time, 'Player', team_name, conf, list(
                        bboxes_p_c[i][:2]), team_name])
                    ...
                    if show_p:
                        ...
                        annotated_frame = cv2.putText(annotated_frame, team_name + f" {conf:.2f}",                  # Add team name annotations
                                                      (int(bboxes_p[i, 0]), int(
                                                          bboxes_p[i, 1])-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                                      color_bgr, 2)
                    ...
                    j += 1
                # Display annotation for other detections (label 1, 2)
                else:
                    ...
                    if detected_ball_src_pos is not None and 'homog' in locals():
                        ...
                        # Save ball data to CSV
                        csv_writer.writerow([frame_nbr, current_time, 'Ball', 'Ball', conf, list(
                            detected_ball_dst_pos[:2]), ''])
            ...
            if show_k:
                for i in range(bboxes_k.shape[0]):
                    ...
                    csv_writer.writerow(
                        [frame_nbr, current_time, 'Keypoint', detected_labels[i], '', list(bboxes_k_c[i][:2]), ''])
            # Plot the tracks
            if len(ball_track_history['src']) > 0:
            ...
            # Save current time to be used in next frame
